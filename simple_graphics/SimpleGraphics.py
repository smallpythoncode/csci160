...
###my addition
#How many time a second will the screen refresh - default is 60/sec
__framerate = 1.0 / 60 #denom is times per second
__nextUpdate = 0
###
...
def __closeClicked(event = None):
  ...
  try:
    __canvas = None
    __master.quit() #added to see if it helps close the shell on ESCAPE
    __master.destroy()
    __master = None
    unregister(__shutdown)
    #exit()#added to see if it helps close the shell on ESCAPE
  finally:
    pass;
...
def setFramerate (framerate):
  global __framerate, __nextUpdate
  __framerate = 1.0 / framerate
  __nextUpdate = 0
...
def update():
  global __nextUpdate, __framerate
  ...
  if __canvas != None:
    ...
    if not __autoupdate:
      if __nextUpdate == 0:
        __nextUpdate = time.time() + __framerate
      while time.time() < __nextUpdate:
        pass;
      __nextUpdate = __nextUpdate + __framerate
...
#  @param g the green component of the color, or None if a named color is used
#  @param b the blue component of the color, or None if a named color is used
def setBackground(r, g=None, b=None):
  global __bgcolor
  ...
  __bgcolor = bg
  if __canvas != None: #just trying it out to avert a crash after pressing ESCAPE
...
def triangle(*pts):
  try:
    ...
    __canvas.create_polygon(new_pts, fill=__fill, outline=__outline, smooth=0, width=__width, joinstyle=__joinstyle)
    __update()
...
def ellipse(x, y, halfWidth, halfHeight):
  try:
    __canvas.create_oval(x + 1 - halfWidth, y + 1 - halfHeight, x+halfWidth, y+halfHeight, fill=__fill, outline=__outline, width=__width)
    __update()
...
def showText(x, y, what, align="nw"):
  try:
...
def setFont(fontName=None, pointSize=10, modifiers=""):
  global __font
  global __font_count
  ...
  if fontName == None:
    ...
    try:
      ...
      __font = font.Font(family=fontName, size=pointSize, name=str(__font_count), weight=w, slant=sl, underline=und, overstrike=ovs)
      __font_count += 1
      return True
    except Exception as e:
      __font = None
